...
$('.showSurveyAnswer').on('click', function () { 
    route = $(this).data('route');
    title = $(this).data('title');
    id = $(this).data('id');
    ...
    if (methodtitle == undefined) {
        title = 'Survey ' + title; //if id =undefined, id = add else = edit
    } else {
        title = methodtitle + ' Modal';
    }

    if (method == undefined) {
        method = id + '/surveyanswer';
    } else 
    { method = id + '/' + method; }
    ...
    $.get("/" + route + '/' + method,
        ...
        function (data, status) { 
            $('#showSurveyAnswer').find('#modal-title')[0].innerHTML = title;
            $('#showSurveyAnswer').find('#modal-body')[0].innerHTML = data;
            $('#showSurveyAnswer').modal();
