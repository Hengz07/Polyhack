...
if (!function_exists('getStaff')) {
    ...
    {
        $endpoint = "hris/staff/profile/v2/$value";
        $client = new \GuzzleHttp\Client();
        $token = config('api.key');
        $url = config('api.url');

        try {
            $response = $client->request('GET', $url . $endpoint, ['query' => [], 'headers' => [
            ...
            return [
                'status' => false,
                'message' => 'Could not find the staff',
            ];

        } catch (\GuzzleHttp\Exception\RequestException $e) {
            return [
                'status' => false,
                'message' => 'Error while get staff profile',
            ];
        }
    }
}
...
if (!function_exists('ldap')) {
     ...
     *
     * */
    function ldap($username, $domain)
    {
        $endpoint = "v1/ldap/search/$username/$domain";
        $client = new \GuzzleHttp\Client();
        $token = config('api.key');
        $url = config('api.url');

        try {
            $response = $client->request('GET', $url . $endpoint, ['query' => ['type'=>'username'], 'headers' => [
                'Authorization' => 'Bearer ' . $token,
            ]]);
            $statusCode = $response->getStatusCode();
            $content = (string) $response->getBody();
